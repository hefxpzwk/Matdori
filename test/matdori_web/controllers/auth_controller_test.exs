defmodule MatdoriWeb.AuthControllerTest do
  ...
  test "POST /auth/logout clears session and redirects to login", %{conn: conn} do
  ...
  test "GET /auth/logout does not log the user out", %{conn: conn} do
    conn = init_test_session(conn, %{google_uid: "google-123", display_name: "Tester"})
    conn = get(conn, ~p"/auth/logout")

    assert redirected_to(conn) == ~p"/login"
    assert get_session(conn, :google_uid) == "google-123"
  end
end
